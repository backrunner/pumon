...
use chrono::Utc;
use procwatch_core::{ManagedProcess, ProcwatchError, ProcwatchResult, ResolvedAppSpec};
use procwatch_platform::state_dir;
...
pub async fn load_processes() -> ProcwatchResult<Vec<ManagedProcess>> {
    tokio::task::spawn_blocking(load_processes_blocking)
        .await
        .map_err(|error| ProcwatchError::Process(format!("state store task failed: {error}")))?
}

pub async fn save_processes(processes: &[ManagedProcess]) -> ProcwatchResult<()> {
...
pub async fn load_desired_apps() -> ProcwatchResult<Vec<ResolvedAppSpec>> {
    tokio::task::spawn_blocking(load_desired_apps_blocking)
        .await
        .map_err(|error| ProcwatchError::Process(format!("state store task failed: {error}")))?
}

pub async fn save_desired_apps(apps: &[ResolvedAppSpec]) -> ProcwatchResult<()> {
...
pub async fn upsert_process(process: ManagedProcess) -> ProcwatchResult<()> {
    tokio::task::spawn_blocking(move || upsert_process_blocking(process))
        .await
        .map_err(|error| ProcwatchError::Process(format!("state store task failed: {error}")))?
}

pub async fn remove_process(name: &str) -> ProcwatchResult<Option<ManagedProcess>> {
...
fn load_processes_blocking() -> ProcwatchResult<Vec<ManagedProcess>> {
    ...
    for row in rows {
        ...
        processes.push(serde_json::from_str(&raw).map_err(ProcwatchError::Json)?);
    }
    Ok(processes)
}

fn save_processes_blocking(processes: &[ManagedProcess]) -> ProcwatchResult<()> {
...
fn load_desired_apps_blocking() -> ProcwatchResult<Vec<ResolvedAppSpec>> {
    let conn = open_connection()?;
    let mut stmt = conn
    ...
    for row in rows {
        let raw = row.map_err(db_error)?;
        apps.push(serde_json::from_str(&raw).map_err(ProcwatchError::Json)?);
    }
    Ok(apps)
}

fn save_desired_apps_blocking(apps: &[ResolvedAppSpec]) -> ProcwatchResult<()> {
    let mut conn = open_connection()?;
    let tx = conn.transaction().map_err(db_error)?;
...
fn upsert_process_blocking(process: ManagedProcess) -> ProcwatchResult<()> {
    let mut conn = open_connection()?;
    let tx = conn.transaction().map_err(db_error)?;
    {
        let mut stmt = tx
...
fn remove_process_blocking(name: &str) -> ProcwatchResult<Option<ManagedProcess>> {
    let mut conn = open_connection()?;
    let tx = conn.transaction().map_err(db_error)?;
    let removed = tx
        .query_row(
        ...
        .optional()
        .map_err(db_error)?
        .map(|raw| serde_json::from_str(&raw).map_err(ProcwatchError::Json))
        .transpose()?;
...
fn persist_process(
    ...
    let json = serde_json::to_string(process).map_err(ProcwatchError::Json)?;
...
fn persist_desired_app(
    ...
    let json = serde_json::to_string(app).map_err(ProcwatchError::Json)?;
    stmt.execute(params![app.name, json]).map_err(db_error)?;
    Ok(())
}

fn open_connection() -> ProcwatchResult<Connection> {
    let dir = state_dir();
    std::fs::create_dir_all(&dir).map_err(ProcwatchError::Io)?;
...
fn ensure_schema(conn: &Connection) -> ProcwatchResult<()> {
    conn.execute_batch(
        r#"
...
fn migrate_legacy_state(conn: &mut Connection) -> ProcwatchResult<()> {
    let version = conn
        .query_row(
...
fn set_schema_version(conn: &Connection) -> ProcwatchResult<()> {
    conn.execute(
...
fn load_legacy_json_vec<T>(path: &Path, backup_prefix: &str) -> ProcwatchResult<Vec<T>>
where
    T: DeserializeOwned,
{
...
fn backup_corrupt_legacy_state(path: &Path, prefix: &str, raw: &str) -> ProcwatchResult<()> {
    ...
    match std::fs::rename(path, &backup) {
        Ok(()) => Ok(()),
        Err(_) => {
            std::fs::write(&backup, raw).map_err(ProcwatchError::Io)?;
...
fn db_error(error: impl std::fmt::Display) -> ProcwatchError {
    ProcwatchError::Process(format!("state store error: {error}"))
}
...
mod tests {
    ...
    struct ProcwatchHomeGuard {
        previous: Option<OsString>,
        home: PathBuf,
    }

    impl ProcwatchHomeGuard {
        fn install(name: &str) -> Self {
            let previous = std::env::var_os("PROCWATCH_HOME");
            let home = std::env::temp_dir().join(format!(
                "procwatch-state-store-{name}-{}-{}",
                std::process::id(),
                Utc::now().timestamp_nanos_opt().unwrap_or_default()
            ));
    ...
    impl Drop for ProcwatchHomeGuard {
        fn drop(&mut self) {
            if let Some(previous) = &self.previous {
                std::env::set_var("PROCWATCH_HOME", previous);
            } else {
                std::env::remove_var("PROCWATCH_HOME");
            }
    ...
    async fn saves_and_loads_processes() {
        ...
        let _guard = ProcwatchHomeGuard::install("roundtrip");
    ...
    async fn migrates_legacy_json_state() {
        let _lock = env_lock().lock().await;
        let _guard = ProcwatchHomeGuard::install("legacy-migration");
    ...
    async fn saves_and_loads_desired_apps() {
        let _lock = env_lock().lock().await;
        let _guard = ProcwatchHomeGuard::install("desired-apps");
