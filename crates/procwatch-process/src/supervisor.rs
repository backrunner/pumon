...
use procwatch_core::{
    ExecMode, ManagedProcess, ProcessStatus, ProcwatchError, ProcwatchResult, ResolvedAppSpec,
};
use procwatch_logging::{ensure_log_paths, spawn_rotating_log_writer};
use procwatch_node_support::{cluster_control_path, resolve_instances, resolve_runtime_command};
use procwatch_platform::{
...
pub async fn start_app(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    start_app_with_log_mode(app, LogCaptureMode::DirectFile).await
}

pub async fn start_app_supervised(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
...
) -> ProcwatchResult<ManagedProcess> {
...
async fn start_app_with_log_mode_and_restart_count(
    ...
    let mut child = child.spawn().map_err(ProcwatchError::Io)?;
    let pid = child
        .id()
        .ok_or_else(|| ProcwatchError::Process(format!("failed to read pid for {}", app.name)))?;
    if matches!(log_mode, LogCaptureMode::PipeRotating) {
...
pub fn validate_restart_policy(app: &ResolvedAppSpec) -> ProcwatchResult<()> {
...
) -> ProcwatchResult<Option<PolicyRestartReason>> {
...
pub fn restart_backoff_remaining(
    ...
    process: &ManagedProcess,
    now: DateTime<Utc>,
) -> ProcwatchResult<Option<Duration>> {
...
fn validate_restart_count(app: &ResolvedAppSpec, next_restart_count: u32) -> ProcwatchResult<()> {
    if let Some(max) = app.restart.max_restarts {
        if next_restart_count > max {
            return Err(ProcwatchError::Process(format!(
...
pub async fn run_app_foreground(app: &ResolvedAppSpec) -> ProcwatchResult<()> {
...
async fn run_app_foreground_inner(
    ...
    loop {
        ...
        let mut child = command_builder.spawn().map_err(ProcwatchError::Io)?;
        let pid = child.id().ok_or_else(|| {
            ProcwatchError::Process(format!("failed to read pid for {}", app.name))
        })?;
        ...
        let status = loop {
            if let Some(status) = child.try_wait().map_err(ProcwatchError::Io)? {
                break status;
            }
                ...
                tokio::select! {
                    changed = shutdown.changed() => {
                        if changed.is_ok() && *shutdown.borrow() {
                            terminate_process_tree(pid).await.map_err(ProcwatchError::Io)?;
                            ...
                            break child.wait().await.map_err(ProcwatchError::Io)?;
                        }
                    }
...
fn start_background_log_writers(
    ...
    log_paths: &procwatch_logging::LogPaths,
) -> ProcwatchResult<()> {
    let stdout = child.stdout.take().ok_or_else(|| {
        ProcwatchError::Process(format!("failed to capture stdout for {}", app.name))
    })?;
    let stderr = child.stderr.take().ok_or_else(|| {
        ProcwatchError::Process(format!("failed to capture stderr for {}", app.name))
    })?;
...
fn foreground_log_writers(
    ...
    child: &mut tokio::process::Child,
    log_paths: &procwatch_logging::LogPaths,
) -> ProcwatchResult<LogWriterHandles> {
    let stdout = child.stdout.take().ok_or_else(|| {
        ProcwatchError::Process(format!("failed to capture stdout for {}", app.name))
    })?;
    let stderr = child.stderr.take().ok_or_else(|| {
...
async fn wait_log_writer(handle: LogWriterHandle) -> ProcwatchResult<()> {
    match handle.await {
        Ok(Ok(())) => Ok(()),
        Ok(Err(error)) => Err(ProcwatchError::Io(error)),
        Err(error) => Err(ProcwatchError::Process(format!(
            "log writer task failed: {error}"
        ))),
    }
}

fn parse_memory_limit(value: &str) -> ProcwatchResult<u64> {
    ...
    let number: u64 = number
        .parse()
        .map_err(|_| ProcwatchError::Config(format!("invalid memory limit: {value}")))?;
...
fn parse_restart_delay(value: &str) -> ProcwatchResult<Duration> {
...
fn parse_duration_ms(value: &str) -> ProcwatchResult<Duration> {
    let trimmed = value.trim();
    let split = trimmed
        .find(|ch: char| !ch.is_ascii_digit())
        .unwrap_or(trimmed.len());
    ...
    let number: u64 = number
        .parse()
        .map_err(|_| ProcwatchError::Config(format!("invalid restart interval: {value}")))?;
...
fn next_cron_delay(value: &str) -> ProcwatchResult<Duration> {
    ...
    for offset in 1..=(366 * 24 * 60) {
        ...
        if cron_matches(&spec, candidate) {
            let delay = candidate
                ...
                .to_std()
                .map_err(|_| ProcwatchError::Config(format!("invalid cron_restart: {value}")))?;
            return Ok(delay);
        }
    }

    Err(ProcwatchError::Config(format!(
...
fn parse_cron_spec(value: &str) -> ProcwatchResult<CronSpec> {
...
fn parse_cron_field(value: &str, min: u32, max: u32) -> ProcwatchResult<Vec<u32>> {
    ...
    for part in value.split(',') {
        ...
        if let Some(step) = part.strip_prefix("*/") {
            let step: usize = step
                .parse()
                .map_err(|_| ProcwatchError::Config(format!("invalid cron step: {value}")))?;
            if step == 0 {
                return Err(ProcwatchError::Config(format!(
        ...
        if let Some((start, end)) = part.split_once('-') {
            let start: u32 = start
                .parse()
                .map_err(|_| ProcwatchError::Config(format!("invalid cron range: {value}")))?;
            let end: u32 = end
                .parse()
                .map_err(|_| ProcwatchError::Config(format!("invalid cron range: {value}")))?;
        ...
        let item: u32 = part
            .parse()
            .map_err(|_| ProcwatchError::Config(format!("invalid cron field: {value}")))?;
...
pub async fn stop_app(name: &str) -> ProcwatchResult<Option<ManagedProcess>> {
...
pub async fn stop_all() -> ProcwatchResult<Vec<ManagedProcess>> {
...
pub async fn restart_app(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    let _ = stop_app(&app.name).await?;
    start_app(app).await
}

pub async fn restart_app_supervised(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    let _ = stop_app(&app.name).await?;
    start_app_supervised(app).await
}

pub async fn reload_app(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    reload_app_with_log_mode(app, LogCaptureMode::DirectFile).await
}

pub async fn reload_app_supervised(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
...
pub async fn scale_app(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    scale_app_with_log_mode(app, LogCaptureMode::DirectFile).await
}

pub async fn scale_app_supervised(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
...
async fn managed_process_for_app(app: &ResolvedAppSpec) -> ProcwatchResult<ManagedProcess> {
    let mut process = load_processes()
        .await?
        .into_iter()
        .find(|process| process.name == app.name)
        .ok_or_else(|| {
            ProcwatchError::Process(format!("managed process not found: {}", app.name))
        })?;
    let command = resolve_runtime_command(app)?;
...
async fn cluster_control(
...
) -> ProcwatchResult<Option<ClusterControlResponse>> {
    ...
    let request = format!(
        "{}\n",
        serde_json::to_string(&wire_request).map_err(ProcwatchError::Json)?
    );
    match timeout(
        Duration::from_secs(3),
    ...
    {
        Ok(Ok(())) => {}
        Ok(Err(error)) => return Err(ProcwatchError::Io(error)),
        Err(_) => {
            let _ = tokio::fs::remove_file(&path).await;
            return Ok(None);
        }
    ...
    let response: ClusterControlResponse =
        serde_json::from_str(&line).map_err(ProcwatchError::Json)?;
    if response.ok {
        Ok(Some(response))
    } else {
        Err(ProcwatchError::Process(response.error.unwrap_or_else(
...
pub async fn list_apps() -> ProcwatchResult<Vec<ManagedProcess>> {
    let mut processes = load_processes().await?;
    let mut system = sysinfo::System::new();
...
pub async fn prune_stale_processes() -> ProcwatchResult<Vec<ManagedProcess>> {
...
mod tests {
    ...
    use procwatch_core::{
        ExecMode, Instances, LogPolicy, RestartPolicy, RuntimeCommand, WatchSpec,
    };
