...
use pumon_config::{find_config, load_config};
use pumon_core::{
    AppSpec, ExecMode, Instances, ManagedProcess, ProcessStatus, PumonConfig, ResolvedAppSpec,
};
use pumon_logging::tail_file;
use pumon_node_support::{resolve_instances, resolve_runtime_command, validate_runtime};
use pumon_platform::{find_program, logs_dir, pumon_home, state_dir};
use pumon_process::{
...
#[derive(Debug, Parser)]
#[command(name = "pumon", version, about = "Rust-first Node.js process manager")]
struct Cli {
...
async fn doctor_report(config: Option<PathBuf>) -> DoctorReport {
    ...
    let directories = vec![
        doctor_directory_report("pumon_home", home.clone()).await,
    ...
    ["node", "npm", "pnpm", "yarn", "bun"]
        ...
        .map(|name| {
            let path = if name == "node" {
                std::env::var_os("PUMON_NODE_PATH")
                    .map(PathBuf::from)
...
fn print_doctor_report(report: &DoctorReport) {
    println!("Pumon home: {}", report.pumon_home.display());
...
async fn daemon_start(config: Option<PathBuf>, json: bool) -> Result<()> {
    ...
    let dir = pumon_home().join("daemon");
    ...
    if json {
        ...
        println!("Started pumon daemon pid={pid}");
    }
    Ok(())
}

async fn daemon_stop(json: bool) -> Result<()> {
    let pid_path = pumon_home().join("daemon").join("daemon.pid");
    let Some(pid) = daemon_pid().await else {
        ...
        if json {
            ...
            println!("pumon daemon not started");
        }
        return Ok(());
    };
    let _ = send_ipc(IpcRequest::Shutdown).await;
    pumon_platform::terminate_process(pid).await?;
    wait_for_process_exit(pid, 30, std::time::Duration::from_millis(100)).await;
    if pumon_platform::is_process_alive(pid) {
        pumon_platform::force_kill_process(pid).await?;
    ...
    if json {
        ...
        println!("Stopped pumon daemon pid={pid}");
    }
    Ok(())
}
...
async fn daemon_status(json: bool) -> Result<()> {
    let pid = daemon_pid().await;
    let running = pid.map(pumon_platform::is_process_alive).unwrap_or(false);
        ...
        tokio::select! {
            _ = tick.tick() => {
                ...
                for app in &apps {
                    ...
                    match process {
                        Some(process) if matches!(process.status, pumon_core::ProcessStatus::Running) => {
...
async fn load_tui_config_apps(config: Option<PathBuf>) -> Result<(Vec<ResolvedAppSpec>, String)> {
    ...
    match resolve_config(None) {
        ...
        Err(_) => Ok((
            Vec::new(),
            "no config loaded; pass pumon tui <config> for start/restart/reload actions"
                .to_string(),
        )),
    }
}
...
fn render_tui(frame: &mut Frame<'_>, model: &TuiModel) {
    ...
    frame.render_widget(
        Paragraph::new(format!(
            "Pumon TUI  |  {} process(es)  |  {} config app(s)",
            model.processes.len(),
...
fn ipc_path() -> PathBuf {
    #[cfg(unix)]
    {
        pumon_home().join("daemon").join("pumon.sock")
    }
    #[cfg(windows)]
    {
        pumon_home().join("daemon").join("pumon.addr")
    }
}
...
fn service_file_path() -> Result<PathBuf> {
    #[cfg(target_os = "macos")]
    {
        ...
        Ok(PathBuf::from(home)
            .join("Library")
            .join("LaunchAgents")
            .join("top.backrunner.pumon.plist"))
    }
    ...
    #[cfg(windows)]
    {
        return Ok(pumon_home().join("service").join("pumon-service.txt"));
    }
}
...
async fn service_status_snapshot() -> Result<ServiceStatusSnapshot> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let label = "top.backrunner.pumon";
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let active =
            command_capture("systemctl", &["--user", "is-active", "pumon.service"]).await?;
        let enabled =
            command_capture("systemctl", &["--user", "is-enabled", "pumon.service"]).await?;
        let daemon_reload = command_capture(
            "systemctl",
            &["--user", "show", "pumon.service", "--property=LoadState"],
        )
        .await?;
...
<plist version="1.0">
<dict>
  <key>Label</key><string>top.backrunner.pumon</string>
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if !status.success {
            let kickstart = command_capture(
                "launchctl",
                &["kickstart", "-k", &format!("{target}/top.backrunner.pumon")],
            )
            .await?;
...
async fn resolve_apps(target: Option<PathBuf>) -> Result<Vec<pumon_core::ResolvedAppSpec>> {
    if let Some(target) = target {
        if target.is_file() && !looks_like_config(&target) {
            ...
            return Ok(pumon_config::normalize_config(
                PumonConfig {
                    apps: vec![app],
                    ..PumonConfig::default()
                },
    ...
    match send_ipc(request).await {
        ...
        Err(error) => {
            let daemon_running = daemon_pid()
                .await
                .map(pumon_platform::is_process_alive)
                .unwrap_or(false);
            if daemon_running {
...
fn collect_snapshot(
    ...
    for entry in std::fs::read_dir(path)? {
        ...
        if matches!(name.as_ref(), ".git" | ".pumon" | "node_modules" | "target") {
            continue;
        }
...
mod tests {
    ...
    use pumon_core::{ExecMode, Instances, LogPolicy, RestartPolicy, WatchSpec};
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let id = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("pumon-{name}-{}-{id}", std::process::id()));
