...
pub async fn load_js_config(path: &Path) -> ProcwatchResult<serde_json::Value> {
    let loader = loader_path()?;
    let node = node_executable()?;
    let selected_env = std::env::var("PROCWATCH_ENV")
        .ok()
    ...
    for args in candidates {
        ...
        let output = command.output().await.map_err(ProcwatchError::Io)?;
        if output.status.success() {
            return serde_json::from_slice(&output.stdout).map_err(ProcwatchError::Json);
        }
...
fn node_executable() -> ProcwatchResult<PathBuf> {
    if let Some(path) = std::env::var_os("PROCWATCH_NODE_PATH").map(PathBuf::from) {
        if path.exists() {
            return Ok(path);
        }
        return Err(ProcwatchError::Config(format!(
            "PROCWATCH_NODE_PATH points to missing file: {}",
            path.display()
        )));
    }

    which("node").map_err(|_| ProcwatchError::Config("node executable not found".to_string()))
}
...
fn loader_path() -> ProcwatchResult<PathBuf> {
    if let Some(path) = std::env::var_os("PROCWATCH_NODE_SUPPORT_LOADER").map(PathBuf::from) {
        if path.exists() {
            return Ok(path);
        }
        return Err(ProcwatchError::Config(format!(
            "PROCWATCH_NODE_SUPPORT_LOADER points to missing file: {}",
            path.display()
        )));
    }
    ...
    let workspace = manifest_dir
        ...
        .ok_or_else(|| ProcwatchError::Config("cannot resolve workspace root".to_string()))?;
