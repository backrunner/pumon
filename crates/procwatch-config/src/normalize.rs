...
use procwatch_core::{
    AppSpec, LogPolicy, ProcwatchConfig, ProcwatchDaemonSpec, ProcwatchError, ProcwatchResult,
    ProcwatchSettings, ResolvedAppSpec, ResolvedConfig,
};

pub fn normalize_config(config: ProcwatchConfig, path: &Path) -> ProcwatchResult<ResolvedConfig> {
    let base = path.parent().unwrap_or_else(|| Path::new("."));
    if config.apps.is_empty() {
        return Err(ProcwatchError::Config(
            "config does not contain apps".to_string(),
        ));
    }

    let procwatch = resolve_procwatch_settings(&config.procwatch, base);
    let default_log = procwatch.log_rotate.clone();
    let apps = config
        .apps
        .into_iter()
        .map(|app| normalize_app(app, base, &default_log))
        .collect::<ProcwatchResult<Vec<_>>>()?;

    Ok(ResolvedConfig { procwatch, apps })
}
...
) -> ProcwatchResult<ResolvedAppSpec> {
    if app.name.trim().is_empty() {
        return Err(ProcwatchError::Config("app name is required".to_string()));
    }
...
fn resolve_procwatch_settings(settings: &ProcwatchSettings, base: &Path) -> ProcwatchSettings {
    let home = settings.home.as_ref().map(|path| absolutize(base, path));
    let node_path = settings
        .node_path
        .as_ref()
        .map(|path| absolutize(base, path));
    let daemon = ProcwatchDaemonSpec {
