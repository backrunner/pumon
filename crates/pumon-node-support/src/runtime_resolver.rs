...
use pumon_core::{ExecMode, Instances, PumonError, PumonResult, ResolvedAppSpec, RuntimeCommand};
use pumon_platform::{find_program, pumon_home};

use crate::{package_manager_from_package_json, read_package_json};

pub fn resolve_runtime_command(app: &ResolvedAppSpec) -> PumonResult<RuntimeCommand> {
...
fn resolve_cluster(app: &ResolvedAppSpec) -> PumonResult<RuntimeCommand> {
    ...
    Ok(RuntimeCommand {
        ...
        args: vec![
            ...
            serde_json::to_string(&spec).map_err(PumonError::Json)?,
        ],
        cwd: app.cwd.clone(),
        env: app.env.clone(),
    })
...
fn resolve_worker_plan(app: &ResolvedAppSpec) -> PumonResult<serde_json::Value> {
    let script = app
        .script
        .as_ref()
        .ok_or_else(|| PumonError::Runtime(format!("cluster app {} requires script", app.name)))?;
    let script_path = absolutize(&app.cwd, script);
    if !script_path.exists() {
        return Err(PumonError::Runtime(format!(
...
fn cluster_shim_path() -> PumonResult<PathBuf> {
    if let Some(path) = std::env::var_os("PUMON_CLUSTER_SHIM").map(PathBuf::from) {
        if path.exists() {
            return Ok(path);
        }
        return Err(PumonError::Runtime(format!(
            "PUMON_CLUSTER_SHIM points to missing file: {}",
            path.display()
        )));
    }
    ...
    let workspace = manifest_dir
        ...
        .ok_or_else(|| PumonError::Runtime("cannot resolve workspace root".to_string()))?;
    let shim = workspace
        .join("packages")
        .join("cluster-shim")
        .join("dist")
...
pub fn validate_runtime(app: &ResolvedAppSpec) -> PumonResult<()> {
    resolve_runtime_command(app).map(|_| ())
}

fn resolve_script(app: &ResolvedAppSpec) -> PumonResult<RuntimeCommand> {
    let script = app
        .script
        .as_ref()
        .ok_or_else(|| PumonError::Runtime(format!("app {} is missing script", app.name)))?;
    let script_path = absolutize(&app.cwd, script);
    if !script_path.exists() {
        return Err(PumonError::Runtime(format!(
            "script not found for {}: {}",
...
fn resolve_package_script(app: &ResolvedAppSpec) -> PumonResult<RuntimeCommand> {
...
fn resolve_custom_command(app: &ResolvedAppSpec, command: &str) -> PumonResult<RuntimeCommand> {
...
fn resolve_program(program: &str, cwd: Option<&Path>) -> PumonResult<PathBuf> {
    ...
    find_program(program, cwd)
        .ok_or_else(|| PumonError::Runtime(format!("program not found: {}", program)))
}
...
fn node_path_override() -> Option<PathBuf> {
    std::env::var_os("PUMON_NODE_PATH").map(PathBuf::from)
}
